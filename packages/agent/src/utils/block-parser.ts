...
export const createBlockParser = ({ pages, emit, emitText }: BlockParserOptions) => {
  ...
  blockResolvers.set('note', {
    ...
    pageType: PageType.MERMAID, // Fallback to MERMAID if note is used to create a page
  })
  ...
  type State = 'idle' | 'await_head' | 'in_block' | 'in_plan' | 'in_tts'
  ...
  const flushPlainText = () => {
    ...
    if (state === 'idle') {
      ...
      // Also check for partial <tts or </tts tags
      else if (buffer.endsWith('<tts')) keepLen = 4
      else if (buffer.endsWith('<tt')) keepLen = 3
      else if (buffer.endsWith('</tts')) keepLen = 5
      else if (buffer.endsWith('</tt')) keepLen = 4
    }
  ...
  // Try to parse tts tags
  const tryParseTTS = (): boolean => {
    // Check for <tts> start tag
    const ttsStartIdx = buffer.indexOf(ttsStartTag)
    if (ttsStartIdx !== -1 && (state === 'idle' || state === 'await_head')) {
      // Emit text before tts tag
      if (ttsStartIdx > 0) {
        const textBefore = buffer.slice(0, ttsStartIdx)
        if (textBefore.length > 0) {
          emitText(textBefore)
        }
      }
      // Remove the <tts> tag from buffer
      buffer = buffer.slice(ttsStartIdx + ttsStartTag.length)
      state = 'in_tts'
      return true
    }

    // Check for </tts> end tag when in tts state
    if (state === 'in_tts') {
      const ttsEndIdx = buffer.indexOf(ttsEndTag)
      if (ttsEndIdx !== -1) {
        const ttsText = buffer.slice(0, ttsEndIdx)
        if (ttsText.length > 0) {
          emit({
            type: 'tts-text',
            options: { text: ttsText },
          } as any)
        }
        buffer = buffer.slice(ttsEndIdx + ttsEndTag.length)
        state = 'idle'
        return true
      } else {
        // Stream tts-text but keep potential end tag
        let keepLen = 0
        if (buffer.endsWith('</tts')) keepLen = 5
        else if (buffer.endsWith('</tt')) keepLen = 4
        else if (buffer.endsWith('</t')) keepLen = 3
        else if (buffer.endsWith('</')) keepLen = 2
        else if (buffer.endsWith('<')) keepLen = 1
  ...
  const ensurePage = (id: string, type: string | null, title?: string) => {
    ...
    // If page doesn't exist, we MUST have a type to create it.
    // If no type is provided, we fallback to MERMAID.
    const finalType = (type || PageType.MERMAID) as PageType
  ...
  return {
    handle(action: ClientAction) {
      ...
      while (true) {
        ...
        if (tryParseTTS()) continue
        // Skip regular parsing when inside plan or tts
        if (state !== 'in_plan' && state !== 'in_tts') {
          tryParse()
        }
