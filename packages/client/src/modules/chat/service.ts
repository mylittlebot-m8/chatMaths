...
export const getChats = async (limit: number, offset: number, userId?: string, type?: string) => {
  try {
    const chats = await db
      ...
      .from(chat)
      .where(
        userId && type ? and(eq(chat.userId, userId), eq(chat.type, type)) :
        userId ? eq(chat.userId, userId) :
        type ? eq(chat.type, type) :
        undefined
      )
...
export const createChat = async (input: string, userId?: string, type?: string) => {
  const [{ id }] = await db
    .insert(chat)
    .values({
      title: 'New Chat',
