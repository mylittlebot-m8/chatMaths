...
import { AgentClientMessage, ClientMessage, GGBClientMessage, isDelayedMessage, MermaidClientMessage, NoteClientMessage, PageCreateClientMessage, PlanClientMessage, TaskClientMessage, UserClientMessage, AvatarVideoClientMessage } from './client-message'
  ...
  return (action: ClientAction | UserAction) => {
    ...
    switch (action.type) {
      ...
      case 'page-create':
        ...
        break
      case 'avatar-video':
        if ('url' in action.options) {
          push({
            type: 'avatar-video',
            id: uuid(),
            url: action.options.url,
          } satisfies AvatarVideoClientMessage)
        }
        break
    }
    return action
