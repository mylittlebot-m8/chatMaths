...
<template>
  ...
  >
    ...
    <textarea
      ...
      :placeholder="isExplaining ? 'Listening to teacher...' : t('chat.placeholder')"
      class="m-2 min-h-[40px] max-h-[200px] w-full bg-transparent outline-none resize-none text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 disabled:opacity-50"
        ...
        <Tooltip>
          <TooltipTrigger as-child>
            ...
            <Button
              variant="ghost"
              size="icon-sm"
              :disabled="uploading || running || isExplaining"
      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger as-child>
            <!-- Send button -->
            <Button
              :disabled="(!running && (input.trim() === '' && resources.length === 0)) || isExplaining"
              size="sm"
              @click="sendUserInput"
            >
              <FontAwesomeIcon
